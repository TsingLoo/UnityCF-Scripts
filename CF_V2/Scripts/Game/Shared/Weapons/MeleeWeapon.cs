...
{
    /// <summary>
    /// Melee Weapon using Point Raycast
    /// refer 3d game kit lite
    /// </summary>
    public class MeleeWeapon : MonoBehaviour
    {
        ...
        private bool CheckDamage(Collider other, AttackPoint pts)
        {
            ...
            if (d.gameObject == m_Owner)
                return true; //ignore self harm, but
                             //do not end the attack

            if ((targetLayers.value
                & (1 << other.gameObject.layer)) == 0)
            {
                //hit an object that is not in our layer,
                //this end the attack. we "bounce" off it
                return false;
            }
