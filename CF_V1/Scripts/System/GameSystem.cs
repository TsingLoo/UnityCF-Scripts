...
public class GameSystem : MonoBehaviour
{
    ...
    [Header("Layers")]
    public LayerMask _grenadeLayer;

    public LayerMask _shootLayer;
    public LayerMask _enemyShootLayer;

    public LayerMask _camera1PLayer;
    public LayerMask _camera1PAimLayer;
    public LayerMask _camera3PLayer;
    ...
    public Dictionary<Guid, WeaponItem> m_WeaponItems;
...
#if UNITY_EDITOR
        //in the editor we find which level we are currently in.
        //Inefficient but since any level can be opened in the
        //editor we can't assume where we start

        string currentScene = SceneManager.GetActiveScene().path;
        for (int i = 0; i < GameDatabase.Instance.episodes.Length 
            && s_CurrentEpisode < 0; ++i)
        {
            for (int j = 0; 
                j < GameDatabase.Instance.episodes[i].scenes.Length; ++j)
            {
                if (GameDatabase.Instance.episodes[i].scenes[j] 
                    == currentScene)
                {
                    s_CurrentEpisode = i;
                    s_CurrentLevel = j;
...
#else
        //in the final game, we init everything to episode & level 0, 
        //as we can't start from somewhere else
    ...
    public void FinishRun()
    {
        ...
        PlayerController.Instance.DisplayCursor(true);
    ...
    {
        var targets = Resources.FindObjectsOfTypeAll<Health>();
    ...
    void SetPickups()
    {
        ...
        var targets = Resources
            .FindObjectsOfTypeAll<WeaponItem>();

        int count = 0;

        foreach (var item in targets)
        {
            if(item.Weapon1P.OwnerPawn == null)
            {
            }
        }
    }
    ...
    void Update()
    {
        ...
        {
            FullscreenMap.Instance
                .UpdateForPlayerTransform(playerTransform);
        }
    }
    ...
    public void EnemyKilled(int score, EDamageType damageType)
    {
        m_TargetDestroyed += 1;
        m_Score += score;

        KillMark.Instance.ShowMark(damageType);//m_TargetDestroyed, count in show mark 
